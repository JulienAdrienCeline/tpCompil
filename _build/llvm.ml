...
let emit_alloca env ?numElts ty =
  ...
  (match numElts with
  | None -> Printf.bprintf env.block "   %s = alloca %a\n" r print_type ty
  | Some v -> Printf.bprintf env.block "   %s = alloca %a, %a\n" r print_type ty print_value_with_type v);
  { ty = Ptr ty; access = r }
...
let rec find_real_src myTree str src1 src2 =
	if (str = src1) then src1
	else (
		if (str = src2) then src2
		else (
			let filsTrue = Hashtbl.find myTree.filsTrue str in
			( match filsTrue with
					Fils(lb) -> find_real_src myTree lb src1 src2
					| NoFils -> 
							let filsSuite = Hashtbl.find myTree.filsSuite str in
							( match filsSuite with
								Fils(lb) -> find_real_src myTree lb src1 src2
								| NoFils -> failwith("Problème pour trouver la source d'un phi")
							)
			)
		)
	)

let rec find_real myTree src1 src2 : (value * value) list -> (value * value) list = function
	(var, lab)::suite ->
		let newLab = {
			ty = lab.ty;
			access = (String.concat "" ["%" ; (find_real_src myTree (String.sub lab.access 1 ((String.length lab.access) - 1)) src1 src2)])
		} in
		(var, newLab)::(find_real myTree src1 src2 suite)
	| [] -> []

let comparaison_pour_remove (variable1,label1) (variable2,label2) =
	if (label1.access = label2.access) then false
	else true

let rec remove_doubles ls =
  match ls with
   | [] -> []
   | h :: t ->
     h :: (remove_doubles (List.filter (comparaison_pour_remove h) t))

let my_find_phis env name myTree src1 src2 =
	let ls = Hashtbl.find env.labs name in
	let ls = find_real myTree src1 src2 ls in
	let ls = remove_doubles ls in
	ls

(* my_emit_phi trouve tous les labels origines d'après le nom source d'une variable *)
let my_emit_phi env name ls =
	let rec print_phis : Buffer.t -> (value * value) list -> unit = fun buffer -> function
		| [x,y] -> Printf.bprintf buffer "[%s, %s]" x.access y.access
    | (x,y)::l -> Printf.bprintf buffer "[%s, %s],%a" x.access y.access print_phis l
    | [] -> ()
