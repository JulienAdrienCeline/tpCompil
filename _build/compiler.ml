...
let rec evalProg (globEnv:genv) (monEnv:env) : labprogramme -> unit = function
	| [] -> ()
	| (AstInstruction(Affectation(Id_name id, expr)))::suite ->
	...
	| (AstInstruction(Print expr))::suite ->
			let v = evalExpr globEnv monEnv expr in
			...
			evalProg globEnv monEnv suite
	| (AstInstruction(Instr_complexe If(expr,sousprog)))::(LlvmInstruction(Label unNoeud))::suite ->
			let macomparaison = evalExpr globEnv monEnv expr in
			let lbtrue = unNoeud.filsTrue.nom in
			let lbfin = unNoeud.filsFalse.nom in
			...
			(* Il faut émettre les phis définis dans le noeud filsFalse *)
			evalProg globEnv monEnv suite
	| _ -> failwith "Not Implemented Yet"
...
let _ = Decap.handle_exception (fun () ->
  let p = doBlock (Decap.parse_channel (programme 0) blank stdin) in 
	...
	match p with (noeudDep, prog) ->
	let lbbefore = noeudDep.nom in
	let _ = emit_br mainEnv lbbefore in
	let _ = emit_block mainEnv lbbefore in
	let _ = evalProg genv mainEnv prog in 
